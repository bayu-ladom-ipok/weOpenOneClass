...
class AugmentedImageSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        batch_x = np.asarray([np.load(image_source_dir+x_path) for x_path in batch_x_path])
        #batch_x = self.transform_batch_images(batch_x)
        batch_y = self.y[idx * self.batch_size:(idx + 1) * self.batch_size]
        return {'input_1':batch_x,'input_2':batch_x},{'leftLayer1': batch_y ,'midLayer1': batch_y, 'rightLayer1': batch_y, 'leftLayer2': batch_y ,'midLayer2': batch_y, 'rightLayer2': batch_y } 
    ...
    def prepare_dataset(self):
        ...
        self.x_path, self.y = df["ImageIndex"].values, df[self.class_names].values
